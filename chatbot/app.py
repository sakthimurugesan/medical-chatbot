...
app = Flask(__name__)
CORS(app, resources={r"/predict": {"origins": "*"}})  # For /predict route only
...
def predict():
    ...
    # Update response if it matches any key in the mapping
    if response not in tag_to_response:
        if response=="greeting":
            response="Welcome to chatbot"
        if response=='end':
            response="Thank You"
        return jsonify({"response": response, "tag": predicted_tag})
    response = tag_to_response.get(response, response)
    url="http://localhost:3000/dept-doctors/"+response
    return jsonify({"response": url, "tag": predicted_tag})
